...
LogManager::LogManager(void) :m_LogLevel(LOG_LEVEL_DEBUG)
{
	...
	m_loggers.insert(std::pair<unsigned int, BaseLogger *>(stdOutLog->GetLogType(), stdOutLog));
}

LogManager::~LogManager(void)
{
	std::map<unsigned int, BaseLogger *>::iterator it = m_loggers.begin();
...
{
	m_loggers.insert(std::pair<unsigned int, BaseLogger*>(log->GetLogType(), log));
}
